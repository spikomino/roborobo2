...
{    
	...
	for ( int i = -2 ; i < +3 ; i++ )
		putPixel32_secured( gScreen, x + i , y , SDL_MapRGBA( gScreen->format, r, g, b, 0 ) );
	for ( int j = -2 ; j < +3 ; j++ )
		putPixel32_secured( gScreen, x , y + j, SDL_MapRGBA( gScreen->format, r, g, b, 0 ) );

	for ( int i = x - dx ; i != x + dx + 1 ; i++ )
	{
		putPixel32_secured( gScreen, i , y - dy , SDL_MapRGBA( gScreen->format, r, g, b, 0 ) );
		putPixel32_secured( gScreen, i, y + dy , SDL_MapRGBA( gScreen->format, r, g, b, 0 ) );
	}

	for ( int j = y - dy ; j != y + dy + 1 ; j++ )
	{
		putPixel32_secured( gScreen, x - dx, j , SDL_MapRGBA( gScreen->format, r, g, b, 0 ) );
		putPixel32_secured( gScreen, x + dx, j , SDL_MapRGBA( gScreen->format, r, g, b, 0) );
	}
}
