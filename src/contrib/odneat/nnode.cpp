...
NNode::NNode (const char *argline)
{
  ...
  char secStr[30], nsecStr[30];
  ss >> secStr >> nsecStr >> nodety >> nodepl;

  struct tm tm;
  strptime(secStr, "%a-%m-%d-%Y-%H:%M:%S", &tm);
  node_id.timestamp.tv_sec = timegm(&tm);

  node_id.timestamp.tv_nsec = atol(nsecStr);
...
void NNode::print_to_file(std::ostream &outFile)
{
    ...
    t1 = gmtime(&(node_id.timestamp.tv_sec));
    /*time1 = asctime(t1);
    time1[strlen(time1)-1] = '\0';*/
    strftime(time1, 30, "%a-%m-%d-%Y-%H:%M:%S", t1);

	char tempbuf[128];
    sprintf(tempbuf, "node %s %.9ld ", &(time1[0]), node_id.timestamp.tv_nsec);
    outFile << tempbuf;
