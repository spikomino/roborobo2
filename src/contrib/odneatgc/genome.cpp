...
Genome::Genome(int id, std::ifstream &iFile) 
{
    ...
    int wordcount = getUnitCountsGc(curline, delimiters);
    int curwordnum = 0;
    ...
    while (!done)
    {
        ...
        {
            iFile.getline(curline, sizeof(curline));
            wordcount = getUnitCountsGc(curline, delimiters);
            curwordnum = 0;
        }
...
{    
    ...
    if (randFloatGc () < mutateAddNodeProbGc)
    {
        //Innovation numbers as gene clocks
        if(!(new_genome->mutate_add_node(newStructureTriesGc,idR,nodeId,gc)))
        {
    ...
    {
        if (randFloatGc () < mutateAddLinkProbGc)
        {
            if (!(new_genome->mutate_add_link(newStructureTriesGc,idR,gc)))
            {
        ...
        else
        {
            ...
            if (randFloatGc () < mutateLinkWeightsProbGc)
            {
                new_genome->mutate_link_weights (sigma);
            }
            if (randFloatGc () < mutateToggleEnableProbGc)
            {
                new_genome->mutate_toggle_enable (1);
            }
            if (randFloatGc () < mutateGeneReenableProbGc)
            {
...
void Genome::mutate_link_weights(double power)
{
    ...
    for(curgene=genes.begin();curgene!=genes.end();curgene++)
    {
        if((*curgene) -> enable)
        {
            ((*curgene)-> lnk) -> weight += power * gaussRandGc();
...
void Genome::mutate_toggle_enable(int times) 
{
    ...
    for (count=1;count<=times;count++) {
        ...
        genenum=randIntGc(0,genes.size()-1);
...
bool Genome::mutate_add_link(int tries,int idR,int &gc)
{
    ...
    if (randFloatGc() < recurOnlyProbGc)
        do_recur=true;
    else do_recur=false;
    ...
    if (do_recur)
    {
        ...
        while(trycount<tries)
        {
            ...
            if (loop_recur)
            {
                nodenum1=randIntGc(first_nonsensor,nodes.size()-1);
                nodenum2=nodenum1;
            }
            else
            {
                //Choose random nodenums
                nodenum1=randIntGc(0,nodes.size()-1);
                nodenum2=randIntGc(first_nonsensor,nodes.size()-1);
            }
    ...
    if (found)
    {
        ...
        while(!done)
        {
            ...
            newweight=randPosNegGc()*randFloatGc()*1.0;
        ...
        else
        {
            ...
            if (p1innov==p2innov)
            {
                ...
                if ((((*p1gene)->enable)==false)||
                        (((*p2gene)->enable)==false))
                    if (randFloatGc()<0.75) disable=true;
...
double Genome::dissimilarity(Genome *g)
{
    ...
    result = (coefEGc * exc/maxLength) + (coefDGc * disj/maxLength) + (coefWGc * weight);
    return result;
}
...
double gaussRandGc()
{
    ...
    if (iset==0)
    {
        do
        {
            v1=2.0*(randFloatGc())-1.0;
            v2=2.0*(randFloatGc())-1.0;
            rsq=v1*v1+v2*v2;
...
double fSigmoidGc(double activesum,double slope)
{
    //NON-SHIFTED STEEPENED
    return (1/(1+(exp(-(slope*activesum)))));
}

int getUnitCountsGc(const char *string, const char *set)
{
    int count = 0;
    short last = 0;
    while(*string)
...
bool load_odneatgc_params(const char *filename, bool output)
{
    ...
    paramFile>>curword;
    paramFile>>mutateOnlyProbGc;

    paramFile>>curword;
    paramFile>>mutateLinkWeightsProbGc;

    paramFile>>curword;
    paramFile>>mutateToggleEnableProbGc;

    paramFile>>curword;
    paramFile>>mutateGeneReenableProbGc;

    paramFile>>curword;
    paramFile>>mutateAddNodeProbGc;

    paramFile>>curword;
    paramFile>>mutateAddLinkProbGc;

    paramFile>>curword;
    paramFile>>mateOnlyProbGc;

    paramFile>>curword;
    paramFile>>recurOnlyProbGc;

    paramFile>>curword;
    paramFile>>newStructureTriesGc;


    if(output)
    {
        printf("mutate_only_prob=%f\n",mutateOnlyProbGc);
        printf("mutate_link_weights_prob=%f\n",mutateLinkWeightsProbGc);
        printf("mutate_toggle_enable_prob=%f\n",mutateToggleEnableProbGc);
        printf("mutate_gene_reenable_prob=%f\n",mutateGeneReenableProbGc);
...
double     mutateOnlyProbGc = 0; // Prob. of a non-mating reproduction
double     mutateLinkWeightsProbGc = 0;
double     mutateToggleEnableProbGc = 0;
double     mutateGeneReenableProbGc = 0;
double     mutateAddNodeProbGc = 0;
double     mutateAddLinkProbGc = 0;
double     mateOnlyProbGc = 0; // Prob. of mating without mutation
