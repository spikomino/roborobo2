...
Genome::Genome(int id, std::ifstream &iFile) 
{
    ...
    int wordcount = getUnitCountsOdNRandom(curline, delimiters);
    int curwordnum = 0;
    ...
    while (!done)
    {
        ...
        {
            iFile.getline(curline, sizeof(curline));
            wordcount = getUnitCountsOdNRandom(curline, delimiters);
            curwordnum = 0;
        }
...
Genome *Genome::mutate(float sigma, int idNewGenome,int &nodeId,int &gc)
{    
    ...
    if (randFloatOdNRandom () < mutateAddNodeProbOdNRandom)
    {
        //Innovation numbers as gene clocks
        if(!(new_genome->mutate_add_node(newStructureTriesOdNRandom,nodeId,gc)))
        {
    ...
    {
        if (randFloatOdNRandom () < mutateAddLinkProbOdNRandom)
        {
            if (!(new_genome->mutate_add_link(newStructureTriesOdNRandom,gc)))
            {
        ...
        else
        {
            ...
            if (randFloatOdNRandom () < mutateLinkWeightsProbOdNRandom)
            {
                new_genome->mutate_link_weights (sigma);
            }
            if (randFloatOdNRandom () < mutateToggleEnableProbOdNRandom)
            {
                new_genome->mutate_toggle_enable (1);
            }
            if (randFloatOdNRandom () < mutateGeneReenableProbOdNRandom)
            {
...
void Genome::mutate_link_weights(double power)
{
    ...
    for(curgene=genes.begin();curgene!=genes.end();curgene++)
    {
        if((*curgene) -> enable)
        {
            ((*curgene)-> lnk) -> weight += power * gaussRandOdNRandom();
...
{
    double result = w;
    if(result < -rangeWOdNRandom)
        result = -rangeWOdNRandom;
    if(result > +rangeWOdNRandom)
        result = +rangeWOdNRandom;
    return result;
}
...
void Genome::mutate_toggle_enable(int times) 
{
    ...
    for (count=1;count<=times;count++) {
        ...
        genenum=randIntOdNRandom(0,genes.size()-1);
...
bool Genome::mutate_add_node(int tries,int &nodeId, int &gc)
{
...
bool Genome::mutate_add_link(int tries,int &gc)
{
    ...
    if (randFloatOdNRandom() < recurOnlyProbOdNRandom)
        do_recur=true;
    else do_recur=false;
    ...
    if (do_recur)
    {
        ...
        while(trycount<tries)
        {
            ...
            if (loop_recur)
            {
                nodenum1=randIntOdNRandom(first_nonsensor,nodes.size()-1);
                nodenum2=nodenum1;
            }
            else
            {
                //Choose random nodenums
                nodenum1=randIntOdNRandom(0,nodes.size()-1);
                nodenum2=randIntOdNRandom(first_nonsensor,nodes.size()-1);
            }
    ...
    if (found)
    {
        ...
        while(!done)
        {
            ...
            newweight=randPosNegOdNRandom()*randFloatOdNRandom()*1.0;
        ...
        else
        {
            ...
            if (p1innov==p2innov)
            {
                ...
                if ((((*p1gene)->enable)==false)||
                        (((*p2gene)->enable)==false))
                    if (randFloatOdNRandom()<0.75) disable=true;
...
double Genome::dissimilarity(Genome *g)
{
    ...
    result = (coefEOdNRandom * exc/maxLength) + (coefDOdNRandom * disj/maxLength) + (coefWOdNRandom * weight);
    return result;
}
...
double gaussRandOdNRandom()
{
    ...
    if (iset==0)
    {
        do
        {
            v1=2.0*(randFloatOdNRandom())-1.0;
            v2=2.0*(randFloatOdNRandom())-1.0;
            rsq=v1*v1+v2*v2;
...
bool load_odneatrandom_params(const char *filename, bool output)
{
    ...
    paramFile>>curword;
    paramFile>>mutateOnlyProbOdNRandom;

    paramFile>>curword;
    paramFile>>mutateLinkWeightsProbOdNRandom;

    paramFile>>curword;
    paramFile>>mutateToggleEnableProbOdNRandom;

    paramFile>>curword;
    paramFile>>mutateGeneReenableProbOdNRandom;

    paramFile>>curword;
    paramFile>>mutateAddNodeProbOdNRandom;

    paramFile>>curword;
    paramFile>>mutateAddLinkProbOdNRandom;

    paramFile>>curword;
    paramFile>>mateOnlyProbOdNRandom;

    paramFile>>curword;
    paramFile>>recurOnlyProbOdNRandom;

    paramFile>>curword;
    paramFile>>newStructureTriesOdNRandom;


    if(output)
    {
        printf("mutate_only_prob=%f\n",mutateOnlyProbOdNRandom);
        printf("mutate_link_weights_prob=%f\n",mutateLinkWeightsProbOdNRandom);
        printf("mutate_toggle_enable_prob=%f\n",mutateToggleEnableProbOdNRandom);
        printf("mutate_gene_reenable_prob=%f\n",mutateGeneReenableProbOdNRandom);
        printf("mutate_add_node_prob=%f\n",mutateAddNodeProbOdNRandom);
        printf("mutate_add_link_prob=%f\n",mutateAddLinkProbOdNRandom);
        printf("mate_only_prob=%f\n",mateOnlyProbOdNRandom);
        printf("recur_only_prob=%f\n",recurOnlyProbOdNRandom);
        printf("newstructure_tries=%d\n",newStructureTriesOdNRandom);
    }
    paramFile.close();
    return true;
}

double     mutateOnlyProbOdNRandom = 0; // Prob. of a non-mating reproduction
double     mutateLinkWeightsProbOdNRandom = 0;
double     mutateToggleEnableProbOdNRandom = 0;
double     mutateGeneReenableProbOdNRandom = 0;
double     mutateAddNodeProbOdNRandom = 0;
double     mutateAddLinkProbOdNRandom = 0;
double     mateOnlyProbOdNRandom = 0; // Prob. of mating without mutation
double     recurOnlyProbOdNRandom = 0;  // Probability of forcing selection of ONLY links that are naturally recurrent
int        newStructureTriesOdNRandom = 0;  // Number of tries mutateAddLink will attempt to find an open link

double     coefEOdNRandom = 1.0;
double     coefDOdNRandom = 1.0;
double     coefWOdNRandom = 0.4;

double     rangeWOdNRandom = 10.0;
