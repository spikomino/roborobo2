...
void Network::print_links_tofile(char *filename) {
	...
	if (!oFile) {
		std::cerr << "[ERROR] Can't open " << filename << " for output" << std::endl;
		exit(-1);
	}
...
//TOFIX: this should be in genome, and take in account genes instead of nodes and links
//For instance, links have not an "enabled" attribute, but genes do.
void Network::drawNetGraphViz(std::string filename)
{
	bool labels = false;
	std::string output = "";
	...
	{
	  if ((inputs[i]->type == SENSOR))
		{	
           	increment = "\n  " + std::to_string(inputs[i]->node_id) +  "\n";		    
			output += increment;
		}
	/*else if (inputs[i]->gen_node_label == BIAS)
		{	
            	    output += "\n    B" + inputs[i]->node_id;		    
		}*/
	  else 
		{
//		    std::cerr << "[ERROR] Wrong input node type " << inputs[i]->gen_node_label << " at position " << i << ", it should be " << INPUT << " or " << BIAS << std::endl;
		    //std::cerr << "[ERROR] Wrong input node type " << inputs[i]->type << " at position " << i << ", it should be " << SENSOR << std::endl;
		    exit(-1);
		}	
	}
	output += "\n  }";
	...
	{
	  if (outputs[i]->type == NEURON)
		{	
            output += "\n  " + std::to_string(outputs[i]->node_id) + "\n";	
		}
	  else 
		{
		   // std::cerr << "[ERROR] Wrong output node type " << outputs[i]->type << ", it should be " << NEURON << std::endl;
		    exit(-1);
		}	
	}
    output += "\n  }\n";
	...
	for(curnode=this->all_nodes.begin();curnode!=this->all_nodes.end();++curnode) 
	{
		if (((*curnode)->type)!=SENSOR) 
		{
			for(curlink=((*curnode)->incoming).begin(); 
				...
				++curlink) 
			{
	        	output += "\n " + std::to_string((*(*curlink)->in_node).node_id);
				output += " -> "+ std::to_string(((*curnode)->node_id));
				strStream << std::setprecision(3);
				//strStream << std::scientific;
				strStream << (*curlink)->weight;
//				output += "[ label=" + std::to_string((*curlink)->weight) + " ]\n";
				if(labels)
					output += "[ label=" + strStream.str() + " ]\n";
				else
					output += "\n";
