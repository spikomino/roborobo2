...
GenomeAdapted *GenomeAdapted::mutate(float sigma, int idRobot ,int idNewGenome, int &nodeId, double &innovNum) 
{
  ...
  new_genome -> setIdTrace(idNewGenome);
  new_genome -> setMom(this -> getIdTrace());
  new_genome -> setDad(-1);  
  ...
  if (randfloat () < NEAT::mutate_add_node_prob){
      ...
      if (new_genome->mutate_add_node
	  (innovations,
	   new_genome->nodeId,
	   new_genome->innovNumber))
...
>>>>>>> 268cc1284e85d0f8ab97d9948403c60564f5c841
    ...
    newgenome=new GenomeAdapted(genome_id,traits_dup,nodes_dup,genes_dup);
