...
def draw_data(exp, text, runs=False, tex=False): 
    ...
    axSp.legend(loc='lower right')
    axSp.set_xlabel('Generations')
    axSp.set_ylabel('#Species')   

    # Average size of controllers
    axSz = subplot2grid((2,4), (1,0))
    c=0
    for e in exp:
        (n, data, stats, species, sizes, stNN) = e
    ...
    axSz.legend(loc='lower right')
    axSz.set_xlabel('Generations')
    axSz.set_ylabel('SizeNN')   

    # average accumulated neural size
    axStNN = subplot2grid((2,4), (1,1))
    stats=[]
    l=[]
    for e in exp:
        (n, d, s,sp,sz,stNN) = e 
        ...
        axStNN.set_title('Avg. Accumulated Neural Size')
