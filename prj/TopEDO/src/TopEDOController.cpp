...
void
TopEDOController::storeBehavior (std::vector < double >in,
...
TopEDOController::createNN ()
{
  ...
  switch (TopEDOSharedData::gControllerType)
    ...
    default:			// default: no controller
      std::
	cerr << "[ERROR] gController type unknown (value: " <<
  ...
  if (dynamic_cast <
      ...
      TopEDOSharedData::gEvaluationTime - 1)
    {
      ...
      std::cout << (gWorld->getIterations () /
		    TopEDOSharedData::gEvaluationTime) << " " << _wm->
	getId () << " " << _currentFitness << " " << _genome->
	getIdTrace () << " " << _genome->getMom () << std::endl;
...
void
TopEDOController::feedBehaviorBase (std::vector < std::pair < std::vector <
				    double >, std::vector < double > > >b)
{
...
double
TopEDOController::dist (std::vector < std::pair < std::vector < double >,
			...
			std::vector < double > > >b2)
{
  if (b1.size () != b2.size ())
    {
      std::cout << "[ERROR] Behaviors of different sizes: b1 (" << b1.
	size () << "), b2 (" << b2.size () << ")" << std::endl;
      exit (-1);
    }
  double res = 0.0;
  double aux = 0.0;
  ...
  for (unsigned int i = 0; i < std::min (b1.size (), b2.size ()); i++)
	...
	{
	  aux +=
	    (b1[i].first[j] - b2[i].first[j]) * (b1[i].first[j] -
						 b2[i].first[j]);
	}
      for (unsigned int j = 0; j < b1[0].second.size (); j++)
	{
	  aux +=
	    (b1[i].second[j] - b2[i].second[j]) * (b1[i].second[j] -
						   b2[i].second[j]);
	}
      res += sqrt (aux);
    }
...
void
TopEDOController::mutate (float sigma)	// mutate within bounds.
{
  ...
  for (count = 0; count < expected_offspring; count++)
	...
	{
	  std::
	    cerr << "[ERROR] EXPECTED OFFSPRING = " << expected_offspring <<
	    " BIGGER THAN POPULATION = " << NEAT::pop_size << std::endl;
	  exit (-1);
	}
      ...
      if ((randfloat () < NEAT::mutate_only_prob) || poolsize == 0)
	{
	  ...
	  if (randfloat () < NEAT::mutate_add_node_prob)
	  ...
	  else if (randfloat () < NEAT::mutate_add_link_prob)
	    {
	      ...
	      std::vector < Innovation * >innovations;
	      if (new_genome->
		  mutate_add_link (innovations, innovNum,
				   NEAT::newlink_tries))
		{
  ...
  if (_wm->isAlive () == true)
      ...
      for (int i = 0; i < _wm->_cameraSensorsNb; i++)
	{
	  ...
	  if (targetIndex >= gRobotIndexStartOffset)
		...
		{
		  std::cerr << "Error from robot " << _wm->
		    getId () << " : the observer of robot " << targetIndex <<
	      ...
	      if (dice <= TopEDOSharedData::gProbaMutation)
		{
		  ...
		  if (dice < 0.5)
		    ...
		    {
		      sigmaSendValue = _currentSigma * 
			  (1 - TopEDOSharedData::gUpdateSigmaStep);	// decrease sigma
