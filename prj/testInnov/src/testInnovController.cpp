...
testInnovController::testInnovController (RobotWorldModel * wm)
{
    ...
    _sigma = testInnovSharedData::gSigmaRef;
...
testInnovController::~testInnovController ()
{
    if(nn != NULL)
        delete nn;
    nn = NULL;
...
testInnovController::reset()
{
    ...
    _energy = testInnovSharedData::gDefaultInitialEnergy;
    _fitness = _energy;
    emptyBasket();
...
void
testInnovController::initRobot ()
{
    _nbInputs = 1;		// Bias constant input (1.0)

    if ((gExtendedSensoryInputs) && (testInnovSharedData::gFitness == 0))
    {
...
unsigned int
testInnovController::computeRequiredNumberOfWeights ()
{
...
testInnovController::step ()
{
    ...
    if ((_energy <=testInnovSharedData::gEnergyThreshold)
...
void
testInnovController::stepBehaviour ()
{
    act();
...
void testInnovController::act()
{
    ...
    for (int i = 0; i < _wm->_cameraSensorsNb; i++)
    {
        ...
        if (testInnovSharedData::gFitness == 1)
        {
            inputs[inputToUse] =
    ...
    inputs[inputToUse++] = _energy / testInnovSharedData::gMaxEnergy;
    ...
    for(int i = 0; i < _wm->_cameraSensorsNb; i++)
    {
        ...
        else if (testInnovSharedData::gFitness == 1)
        {
            if(_md > inputs[2 * i] &&
...
void
testInnovController::updateFitness ()
{
    switch(testInnovSharedData::gFitness)
    {
    case 0:
...
void testInnovController::updateFitnessNavigation(){
    _fitnessUpdateCounter++;
    if(_fitnessUpdateCounter >= testInnovSharedData::gFitnessFreq)
    {
...
void testInnovController::updateFitnessForaging(){
    _fitnessUpdateCounter++;
    if(_fitnessUpdateCounter >= testInnovSharedData::gFitnessFreq)
    {
        _genome->nbFitnessUpdates++;
...
void
testInnovController::broadcastGenome ()
{
    std::vector<testInnovController *> neighbors;
    ...
    for (int i = 0; i < _wm->_cameraSensorsNb; i++)
    {
        ...
        if (targetIndex >= gRobotIndexStartOffset)
        {
            ...
            testInnovController *targetRobotController =
                    dynamic_cast <
                    testInnovController *
...
void testInnovController::storeMessage(message msg){
...
void testInnovController::emptyGenomeList(){
    population.clear();
    species.clear();
}
void testInnovController::pickItem(){
    _items++;
    _energy += testInnovSharedData::gEnergyItemValue;
}
void testInnovController::emptyBasket(){
    _items = 0;
}
void testInnovController::gatherEnergy()
{
    _energy = std::max(0.0,std::min(testInnovSharedData::gEnergyItemValue + _energy,testInnovSharedData::gMaxEnergy));
}
...
void
testInnovController::stepEvolution ()
{
...
void testInnovController::logGenome()
{

    //GENERATION ID-ROBOT FITNESS SIZE(localPop) IDGENOME IDMOM
    testInnovSharedData::gEvoLog <<
                                 dynamic_cast <testInnovWorldObserver *>
    ...
    filename = testInnovSharedData::gGenomeLogFolder;
...
int
testInnovController::selectBest ()
{
...
int
testInnovController::selectRankBased()
{       
    std::map < int, message>::iterator it = population.begin();
...
int
testInnovController::selectBinaryTournament()
{
    int result = -1;
...
int
testInnovController::selectRandom()
{       
...
bool testInnovController::compareFitness(std::pair<int,float> i,std::pair<int,float> j)
{
    return (i.second < j.second);
}

void testInnovController::printIO( std::pair< std::vector<double>, std::vector<double> > io)
{
...
void testInnovController::printVector(std::vector<double> v)
{
...
void testInnovController::printFitnessList()
{
    std::cout << "------------------------------" << std::endl;
    std::map<int,message>::iterator it = population.begin();
...
void testInnovController::save_genome(){
    char fname[128];
    snprintf(fname, 127, (testInnovSharedData::gGenomeLogFolder+"%04d-%010d.gen").c_str(),
...
void testInnovController::print_genome(Genome* g){
...
void testInnovController::printRobot(){
...
void testInnovController::printAll(){
    printRobot();
    print_genome(_genome);
    std::cout << "\n";
}
bool testInnovController::lifeTimeOver(){
    return get_lifetime()
            >= testInnovSharedData::gEvaluationTime - 1;
}

int testInnovController::get_lifetime(){
    return dynamic_cast <testInnovWorldObserver*>
            (gWorld->getWorldObserver())->getLifeIterationCount();
}
bool testInnovController::doBroadcast(){
    bool result = false;
...
bool testInnovController::in_maturation_period(){

    bool result = false;
    if(gWorld->getIterations () <=
            _birthdate + testInnovSharedData::gMaturationPeriod)
        result = true;

    return result;

}
bool testInnovController::tabu_list_approves(Genome* g)
{
    bool result = true;
    ...
    for(;it != tabuEnd;it++)
    {
        if(std::get<0>(*it)->dissimilarity(g) < testInnovSharedData::gTabuThreshold)
        {
            std::pair<Genome*, int> & pair  = (*it);
            std::get<1>(pair) = testInnovSharedData::gTabuTimeout;
...
bool testInnovController::population_accepts(message msg)
{
    bool result = false;

    if(population.size() < testInnovSharedData::gMaxPopSize)
    {
        result = true;
    }
    else
...
void testInnovController::add_to_tabu_list(Genome* g)
{
    int idx = tabu_contains(g);
    if( idx == -1)
        tabu.push_back(std::make_pair(g, testInnovSharedData::gTabuTimeout));
    else
        std::get<1>(tabu[idx]) = testInnovSharedData::gTabuTimeout;
}

void testInnovController::add_to_population(message msg)
{
...
void testInnovController::add_unconditional_to_population(message msg)
{
    ...
    else //new genome
    {
        ...
        else
        {
            ...
            if(tabu_contains(worseGenome) == -1)
            {
                ...
                tabu.push_back(std::make_pair(worseGenome, testInnovSharedData::gTabuTimeout));
            }
            else
            {
...
int testInnovController::tabu_contains(Genome* g)
{
    int result = -1;
    std::vector<std::pair<Genome*,int> >::iterator it = tabu.begin(), tabuEnd =  tabu.end();
    for(; it < tabuEnd; it++)
...
int testInnovController::findInPopulation(Genome* g)
{
    int result = -1;
    int id = g->genome_id;
    std::map<int,message>::iterator it = population.begin();
...
int testInnovController::findInSpecies(Genome* g)
{
    int result = -1;
...
void testInnovController::add_to_species(message msg)
{
...
int testInnovController::computeSpeciesId(Genome* g)
{
    std::map<int,std::pair<std::set<Genome*>,double>>::iterator it = species.begin();
    ...
    for(; it != species.end(); it++)
    {
        ...
        if(g->dissimilarity(*itSp) < testInnovSharedData::gCompatThreshold)
        {
            result = it -> first;
            break;
        }
...
void testInnovController::recomputeAllSpecies()
{
    species.clear();
    ...
    if(i > testInnovSharedData::gMaxPopSize)
    {
        std::cerr << "[ERROR] Excess in population : " << i << "/" << testInnovSharedData::gMaxPopSize  << std::endl;
        exit(-1);
    }
}

void testInnovController::adjust_population_size()
{    
    //Not needed, already done in add_to_population()
}
void testInnovController::adjust_species_fitness()
{
    std::map<int,std::pair<std::set<Genome*>,double>>::iterator it = species.begin();
    std::set<Genome*>::iterator itSp;
    double adjFit = 0.0;
...
void testInnovController::adjust_active_species_fitness(int spId)
{
    std::set<Genome*>::iterator itSp;
    double adjFit = 0.0;
    //iterate over the individuals of current species
...
Genome* testInnovController::generate_offspring()
{
    Genome* result = NULL;
    ...
    int newId = _wm->getId () + 10000 * (1 + (gWorld->getIterations () /
                                              testInnovSharedData::gEvaluationTime));
...
int testInnovController::selectSpecies()
{
    int result = -1;
    double totalAdjFitness = 0.0;
    std::map<int,std::pair<std::set<Genome*>,double>>::iterator it = species.begin();
...
Genome* testInnovController::selectParent(int spId)
{
...
void testInnovController::update_population(Genome* offspring)
{
    //Nothing to do (?)
}
double testInnovController::update_energy_level()
{
    double result = 0.0;
...
double testInnovController::updateEnergyForaging()
{
    double result = _energy;
    //Fixed rate of energy consumption
    //Energy gathering at energy point done in agent observer
    result -= testInnovSharedData::gEnergyConsumption;

    //cap energy (in [0,maxEnergy])
    return std::max(0.0,std::min(result,testInnovSharedData::gMaxEnergy));
}
double testInnovController::updateEnergyNavigation()
{
    double result = 0.0;
    ...
    //cap energy (in [0,maxEnergy])
    return std::max(0.0,std::min(result + _energy,testInnovSharedData::gMaxEnergy));
}
void testInnovController::cleanPopAndSpecies()
{
    bool ok = true;
