...
TestController::TestController( RobotWorldModel *wm )
{
  ...
  _currentSigma = TestSharedData::gSigmaRef;
...
void TestController::resetRobot()
{
  ...
  if ( TestSharedData::gEnergyRequestOutput )
    _nbOutputs += 1; // incl. energy request


  _nbHiddenLayers = TestSharedData::gNbHiddenLayers;


  //_nbNeuronsPerHiddenLayer = new std::vector<unsigned int>(_nbHiddenLayers);

  for(unsigned int i = 0; i < _nbHiddenLayers; i++)
    _nbNeuronsPerHiddenLayer[i] = TestSharedData::gNbNeuronsPerHiddenLayer;
  ...
  for ( unsigned int i = 0 ; i != nbGene ; i++ )
    {
      _genome.push_back((double)(rand()%TestSharedData::gNeuronWeightRange)/(TestSharedData::gNeuronWeightRange/2)-1.0); // weights: random init between -1 and +1
    }
...
void TestController::stepBehaviour()
{
...
void TestController::storeBehavior(std::vector<double> in,std::vector<double> out)
{
...
float TestController::updateFitness(std::vector<double> in,std::vector<double> out)
{
  float deltaFit = 0.0;
...
float TestController::dist(float x1, float y1, float x2, float y2)
{
  float result = 0.0;
...
void TestController::createNN()
{
  if ( nn != NULL ) // useless: delete will anyway check if nn is NULL or not.
    delete nn;
  
  switch ( TestSharedData::gControllerType )
    {
    case 0:
      {
	// MLP
    ...
    default: // default: no controller
      std::cerr << "[ERROR] gController type unknown (value: " << TestSharedData::gControllerType << ").\n";
      exit(-1);
    };
}


unsigned int TestController::computeRequiredNumberOfWeights()
{
...
void TestController::stepEvolution()
{
  ...
  if( dynamic_cast<TestWorldObserver*>(gWorld->getWorldObserver())->getLifeIterationCount() >= TestSharedData::gEvaluationTime-1 )
    {
      float r = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
     
      if(r < TestSharedData::gProbReEval)
	{
           //Reevaluate champion
...
void TestController::feedBehaviorBase(std::vector<std::pair< std::vector<double>, std::vector<double> > > b)
{
  ...
  for(i = 0; i < _behBase.size() ; i++)
   ...
   if(minDist < TestSharedData::gBehThresh)
    {
...
double TestController::dist(std::vector<std::pair< std::vector<double>, std::vector<double> > > b1, std::vector<std::pair< std::vector<double>, std::vector<double> > > b2)
{
  if(b1.size() != b2.size())
    {
...
void TestController::selectRandomGenome()
{
...
void TestController::storeGenome(std::vector<double> genome, int senderId, int senderBirthdate, float sigma)
{
...
void TestController::mutate( float sigma) // mutate within bounds.
{
  _genome.clear();
...
void TestController::broadcastGenome()
{
}

void TestController::loadNewGenome()
{
  // Logging
  ...
  int selected;
      ...
      for(unsigned int i=0; i<_genome.size(); i++)
	{
      ...
      if(selected == 0)
	{
	  _currentSigma = std::min((double)_currentSigma * 2, TestSharedData::gSigmaMax);
	}
      else 
	if(selected == 1)
	{
	  _currentSigma = TestSharedData::gSigmaMin;
	}
	else //DEBUG
	  {
...
int TestController::selectBest(std::map<int,float> lFitness)
{
  int idx = -1;
